...
import { setClanLogClan, setClanLogClanEnabled } from "../../../actions/server"
import ChannelDropdown from "../home/channel-dropdown"

export default function ClanLogsModal({ channels, clan, enabled, id }) {
  const [error, setError] = useState("")
  const [loading, setLoading] = useState(false)
  const [opened, { close, open }] = useDisclosure(false)
  const [channelId, setChannelId] = useState(null)
  const [showEnabled, setShowEnabled] = useState(!clan?.clanLogs?.enabled || !clan?.webhookUrl1 || !clan?.webhookUrl2)

  const sendEnableNotifications = () => {
    notifications.show({
      autoClose: 8000,
      color: "green",
      message: `Clan Logs successfully enabled for ${clan.clanName}.`,
      title: `Clan Logs Enabled!`,
    })
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await setClanLogClan({ channelId, guildId: id, tag: clan.tag })
  ...
  const handleDisable = async () => {
    const { success } = await setClanLogClanEnabled(clan.tag, false)
    if (success) setShowEnabled(true)
  }

  return (
    <>
      <Modal centered onClose={close} opened={opened} title={<Title fz="1.5rem">Enable Clan Logs</Title>}>
        <Stack gap="md">
