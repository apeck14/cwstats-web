...
import { deleteLinkedClan, postDailyWarReport, postSeasonalReport } from '@/actions/server'
...
export default function LinkedClanCard({ channels, clan, clans, id, setClans, webhookExists }) {
  ...
  const handleReportUpdate = async (type, enabled) => {
    ...
    if (!error) {
      ...
      notifications.show({
        autoClose: 8000,
        color: 'green',
        message: `${label} successfully ${enabled ? 'enabled' : 'disabled'} for ${clan.clanName}.`,
      ...
      sendLogWebhook(
        {
          clan: clan.clanName,
          color: embedColors[enabled ? 'green' : 'red'],
          guild: id,
          tag: formatTag(clan.tag, true),
          title: `${label} ${enabled ? 'Enabled' : 'Disabled'}`
        },
        ...
        <Accordion.Panel>
          <Stack gap='xs'>
            ...
            <SimpleGrid cols={{ base: 1, md: 3 }}>
              ...
              >
                {isPlus && webhookExists && seasonalReportEnabled ? (
                  <Button
                    color='red'
                    maw='fit-content'
                    onClick={() => handleReportUpdate('seasonal', false)}
                    size='xs'
                    variant='light'
                  >
                    Disable
              ...
              >
                {isPlus && webhookExists && warReportEnabled ? (
                  <Button
                    color='red'
                    disabled={!isPlus}
                    maw='fit-content'
                    onClick={() => handleReportUpdate('war', false)}
                    size='xs'
                    variant='light'
                  >
                    Disable
          ...
          {showConfirmButtons ? (
            <Group gap='xs' justify='end'>
              <ActionIcon color='green' onClick={handleDelete}>
