...
export default function ClanLogsOverview({ clanTag, log }) {
	...
	return (
		Object.keys(groupedLogsBySeason).reverse().map((sId, sIndex) => {
			return (
					...
					<tbody>
						{
							groupedLogsBySeason[sId].map((w, index) => {
								...
								return (
									<Row key={index} onClick={() => handleClickScroll(w.seasonId, w.sectionIndex)}>
										<Cell>W{w.sectionIndex + 1}</Cell>
										<Cell align="center">{clan.rank}</Cell>
										<Cell align="center">{clan.clan.fame}</Cell>
										<Cell align="center">{getAvgFameOfLog(clan.clan, w.createdDate, clan.clan.finishTime).toFixed(1)}</Cell>
										<Cell align="right">
											<span style={{
												color: gray["25"],
												marginRight: "0.25rem"
