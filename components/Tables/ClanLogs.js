...
const ContentHeaderDiv = styled.div({
	...
	"background": "linear-gradient(90deg, rgba(255,165,0,1) 0%, rgba(255,35,122,1) 100%)",
	"color": gray["0"],
	"alignItems": "center",
	"marginTop": "1rem",
		...
		<>
...

			{
				log.map((w, index) => {
					...
					return (
						<ContentDiv key={index} id={`${w.seasonId}-${w.sectionIndex}`}>
							<ContentHeaderDiv>
								<HeaderText>Season {w.seasonId} - Week {w.sectionIndex + 1}</HeaderText>
								<Date>{parseDate(w.createdDate).toUTCString()}</Date>
							</ContentHeaderDiv>
							...
							<ClansDiv>
								{
									w.standings.map((c, index) => {
										...
										return (
											<ClanItem isClan={isClan} key={index} onClick={() => isClan ? null : router.push(`/clan/${c.clan.tag.substring(1)}/log`)}>
												<ClanItemText flex={isMobile ? 0.15 : 0.25}>{c.rank}</ClanItemText>
												<ClanItemText flex={isMobile ? 0.85 : 0.75}>
													<ClanBadge src={`/assets/badges/${getClanBadgeFileName(c.clan.badgeId, c.clan.clanScore)}.png`} height={clanBadgeHeight} width={clanBadgeWidth}/>
													{c.clan.name}
												</ClanItemText>
												<ClanItemText flex={0.5} alignRight={true}>
													{c.clan.fame}
