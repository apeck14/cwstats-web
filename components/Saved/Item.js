...
export default function SavedItem({ name, tag, badge, links, isPlayer }) {
	...
	return (
		<ContentItem>
			<IconDiv>
				<Icon src={iconSrc} alt="Icon" width={isPlayer ? profileIconWidth : clanBadgeWidthPx} height={isPlayer ? profileIconHeight : clanBadgeHeightPx} />
			</IconDiv>
			<LeftDiv>
			...
			<RightDiv>
				{
					links.map((l, index) => {
						return (
							<ContentLink key={index} href={l.url}>{l.name}</ContentLink>
						)
					})
				}
			</RightDiv>
