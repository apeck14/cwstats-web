...
export const getAvgFameOfLog = (clan, weekEnd, finishTime, isColosseum = false) => {
  const weekEndDate = parseDate(weekEnd)
  const finishDate = parseDate(finishTime)

  weekEndDate.setUTCDate(weekEndDate.getUTCDate() - 1)

  const threeDayFinish = isColosseum ? false : weekEndDate.getUTCDate() === finishDate.getUTCDate()
