...
export default function ClanRace({ clan, race }) {
    ...
    const getShownParticipants = () => {
        ...
        return race.clan.participants
            .filter(
                (p) =>
                    clan.memberList.find((m) => m.tag === p.tag) || p.fame > 0,
            )
            .sort((a, b) => {
        ...
        <>
            <NextSeo
                ...
                openGraph={{
                    title: `${clan.name} | Race - CWStats`,
                    description:
                        "View current river race stats and projections.",
                    images: [
                        {
            ...
            <Main>
                ...
                <NavDiv>
                    <NavItem
                        onClick={() =>
                            router.push(`/clan/${clan.tag.substring(1)}`)
                        }
                    >
                        Home
                    </NavItem>
                    ...
                    <NavItem
                        onClick={() =>
                            router.push(`/clan/${clan.tag.substring(1)}/log`)
                        }
                    >
                        Log
                    </NavItem>
                    <NavItem
                        onClick={() =>
                            router.push(`/clan/${clan.tag.substring(1)}/stats`)
                        }
                    >
                        Stats
                    </NavItem>
                    ...
                    <>
                        <RaceDiv>
                            {raceClans.map((c, index) => {
                                const clanInRace = race.clans.find(
                                    (cl) => cl.tag === c.tag,
                                )
                        ...
                        <RaceLeaderboard
                            participants={getShownParticipants()}
                        />
                    </>
                )}
            </Main>
